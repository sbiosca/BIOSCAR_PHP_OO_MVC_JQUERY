function ajaxForSearch(url, items = 0, total) {
    //var total = 0;
    ajaxPromise(url, 'GET' , 'JSON', {total: total,items: items})
    .then(function(data) {
...
function load_filter() {
    ajaxPromise('modules/shop/ctrl/ctrl_shop.php?op=filters', 'GET', 'json')
    .then(function(data) {
            $(".form").append(
...
function filters() {
    ...
    document.filter.action="index.php?modules=modules/shop/ctrl/ctrl_shop&op=list_shop";
}
...
function remove_filters() {
    $(document).on('click', '#remove-filters', function() {
        ...
        localStorage.removeItem('filters');
        window.location.href = 'index.php?modules=modules/shop/ctrl/ctrl_shop&op=list_shop';
    });
}
...
function details(id) {
        ajaxPromise('modules/shop/ctrl/ctrl_shop.php?op=list_one_car&id=' + id, 'GET', 'json')
       .then(function(data) {
        console.log(data);
        if (data[row].date){
            ...
            $('<div></div>').attr('class',"services__content").appendTo(".button1")
                  .html("<button><a href='index.php?modules=modules/shop/ctrl/ctrl_shop&op=list_shop'>VOLVER</a></button>")
            mapBox(data);
            more_cars(data);
        }).catch(function() {
            window.location.href = 'index.php?modules=modules/exceptions/ctrl/ctrl_exceptions&err=503';
...
function count(id) {
    ajaxPromise('modules/shop/ctrl/ctrl_shop.php?op=count&id=' + id , 'GET', 'json')
}
...
function load_pagination(url2, url){
   console.log(url2);
    ajaxPromise(url2, 'GET', 'json')
    .then(function(data) {
